...
class LFBot:
    def __init__(self, X, y, feature_names, categorical_features, class_names, parameters, description, lf,
        ...
        for feature in feature_names:
            ...
            if feature in categorical_features:
                ...
                self.feature_values[i].append(lf.min_max_feature_values[feature][0])
                self.feature_values[i].append(lf.min_max_feature_values[feature][1])
            i += 1
        ...
        self.mean_values = []
        for f in self.lf.min_max_feature_values:
            self.mean_values.append(self.lf.min_max_feature_values[f][2])
    ...
    def run(self):
        ...
        def get_lf_prediction():
            self.instance_after_change = [float(i) for i in request.args.getlist('instance[]')]
            if not self.instance_after_change:
                return 'failure'  # maybe render a failure.html or something
            else:
                self.accuracy = 0
                self.lf_rule = self.lf.explain(self.instance_after_change, True)
