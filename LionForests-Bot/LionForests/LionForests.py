...
class LionForests:
    ...
    def fit_trained(self, train_data, train_target):
        ...
        for i in range(len(self.feature_names)):
            self.min_max_feature_values[self.feature_names[i]] = [
                min(train_data[:, i]), max(train_data[:, i]), sum(train_data[:, i])/len(train_data[:, i]),np.array(train_data[:, i]).std()]
    ...
    def calculate_single_feature_values(self, instance, feature, rule, discrete_features):
        ...
        min_value, max_value = self.min_max_feature_values[feature][0], self.min_max_feature_values[feature][1]
        mean_, std_ = self.min_max_feature_values[feature][2], self.min_max_feature_values[feature][3]
